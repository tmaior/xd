...
def train_rel(**context):
    ...
    if dataset_type == 'multiTable':
        ...
        for table in tables:
            with mlflow.start_run() as active_run:
                ...
                mlflow_pyfunc_model_path = "gc_model_" + str(table)
                numOfRows.append(tables[table].shape[0])

                model = GaussianCopula()
                logger.info(f"modeling {table}")
                model.fit(tables[table])
                model_path = airflow_data_folder + '/models/model_'+str(table)+'.pkl'
                ...
                model_uri.append(mlflow.get_artifact_uri(mlflow_pyfunc_model_path))
    ...
    shutil.copy(airflow_data_folder+ '/' + 'data_orig/' + metadata_path, airflow_data_folder + '/' + 'data_synth/')  
