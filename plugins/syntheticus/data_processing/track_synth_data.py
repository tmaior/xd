...
def track_synth_data(**context):
    ...
    if not os.popen("dvc status").read() == "Data and pipelines are up to date.\n":
        logger.info("Synthetic data update detected")
        ...
        commit_msg = ' '.join(["Adding synthetic dataset version",dataset_dir,timestamp])
        sp.Popen(f"git commit -m '{commit_msg}'", shell=True).wait()
        logger.info(f"Committed new synthetic dataset version {timestamp}")
        
        sp.Popen("dvc push", shell=True).wait()
        logger.info("Pushed data to remote")
    else:
        logger.info("Synthetic data did not change. Nothing to track.")